...
function FunctionItems({
    ...
    // Only top-level uses controlled open, subfunctions use local state
    const [localOpen, setLocalOpen] = useState(false);
    const open =
        level === 0 && controlledOpen !== undefined
            ? controlledOpen
            : localOpen;
    const setOpen =
        level === 0 && controlledSetOpen ? controlledSetOpen : setLocalOpen;
    ...
    useEffect(() => {
        if (!searchTargetId) return; // Do nothing on load
...
export default function FunctionalStructure() {
    const [openTopLevelId, setOpenTopLevelId] = useState(null);
    const navigate = useNavigate();
    const {
        data: functionData,
        isLoading,
    ...
    return (
        <section className="w-auto p-6 lg:m-2 bg-white rounded-lg shadow">
            ...
            {functionData?.map((data, idx) => {
                const id = data.id || data.subfunction_id;
                return (
                    <FunctionItems
                        key={idx}
                        data={data}
                        ...
                        onRef={handleRef}
                        refetch={refetch}
                        open={openTopLevelId === id}
                        setOpen={() =>
                            setOpenTopLevelId(openTopLevelId === id ? null : id)
                        }
                    />
                );
            })}
